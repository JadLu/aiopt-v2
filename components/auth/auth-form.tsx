...
import { signIn, signUp, signInWithGoogle, getGoogleRedirectResult, resetPassword, mapFirebaseError } from "@/lib/firebase/auth";
import { initUserCredits } from "@/lib/firebase/credits";
...
export function AuthForm() {
  ...
  useEffect(() => {
    ...
    getGoogleRedirectResult()
      .then(async (result) => {
        if (result?.user) {
          await initUserCredits(result.user.uid);
      ...
      .catch((err: unknown) => {
        ...
        if (code) setError(mapFirebaseError(code));
        setGoogleLoading(false);
      });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "login") {
        ...
        setSuccessMsg("Reset link sent! Check your inbox — and your spam folder if you don't see it within a minute.");
      }
    } catch (err: unknown) {
      const code = (err as { code?: string }).code ?? "";
  ...
  async function handleGoogle() {
    ...
    setGoogleLoading(true);
    try {
      // signInWithGoogle triggers a full-page redirect; page will come back
      // and the useEffect above will pick up the result via getGoogleRedirectResult()
  ...
  return (
    <div className="glass-card fade-up" style={{ borderRadius: "var(--r-hero)", padding: "36px", width: "100%", maxWidth: "440px" }}>
      {/* Logo / Brand */}
